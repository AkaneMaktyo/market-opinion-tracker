...
主发布使用支持断点续传的 SFTP 流水线上传，并在日志中记录每个文件的耗时和平均速度。不要改回逐块等待确认的串行写入；公网 SSH 的往返延迟会被大量 32 KiB 请求放大，导致后端包上传从分钟级恶化到十几分钟。若发布再次变慢，先比较日志中的上传速度、SSH 重试次数和 Windows 部署机到云服务器的网络质量。
