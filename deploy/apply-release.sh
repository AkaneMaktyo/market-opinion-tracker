...
if [[ -n "$RUNTIME_ENV_PATH" && -f "$RUNTIME_ENV_PATH" ]]; then
  while IFS= read -r line; do
    ...
    value="${line#*=}"
    case "$key" in
      PRICE_ALERT_WXPUSHER_SPT|SEC_USER_AGENT|BINANCE_SPOT_API_KEY|BINANCE_SPOT_KEY_TYPE|\
      ...
      BITGET_ENABLED|BITGET_PAPER_TRADING|BITGET_API_KEY|BITGET_API_SECRET|\
      BITGET_PASSPHRASE|BITGET_PRODUCT_TYPE|BITGET_MARGIN_COIN)
        temp_env="$(mktemp)"
