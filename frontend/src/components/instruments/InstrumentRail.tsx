...
import { applyManualOrder, groupItems, mergeDefaultInstrument, parseMode, sortItems, sortOptions, type SortMode } from './instrumentList';
...
export function InstrumentRail({ instruments, selected, groups, onSelect, onChanged }: Props) {
  ...
  useEffect(() => {
    try {
      ...
      setGroupOrder(JSON.parse(window.localStorage.getItem(GROUP_ORDER_STORAGE_KEY) || '[]'));
  ...
  useEffect(() => {
    setGroupOrder((current) => {
      const next = [...current.filter((group) => groups.includes(group)), ...groups.filter((group) => !current.includes(group))];
  ...
  function dropGroupOn(event: DragEvent<HTMLButtonElement>, target: string) {
    event.preventDefault();
    if (!draggingGroup || draggingGroup === target) return setDraggingGroup('');
    const current = groupOrder.filter((group) => group !== draggingGroup);
    const rect = event.currentTarget.getBoundingClientRect();
    const insertAt = current.indexOf(target) + (event.clientY > rect.top + rect.height / 2 ? 1 : 0);
  ...
  return (
    <aside className="rail">
      <div className="rail-head">
        <div><div className="panel-title">鍝佺</div><span className="rail-note">{mode === 'manual' ? '鎷栧姩鎺掑簭' : '琛屾儏鎺掑簭'}</span></div>
        <button className="rail-manage" onClick={() => setDirectoryOpen(true)} type="button"><FolderTree size={14} /><span>绠＄悊</span></button>
      </div>
      <div className="rail-sort">
        {sortOptions.map((option) => {
          const Icon = sortIcons[option.value];
          return <button className={mode === option.value ? 'sort-button active' : 'sort-button'} key={option.value} onClick={() => { setMode(option.value); window.localStorage.setItem(SORT_STORAGE_KEY, option.value); }} type="button"><Icon size={14} /><span>{option.label}</span></button>;
        })}
      </div>
      <div className="rail-table-head"><span>鍟嗗搧</span><span>鏈€鏂颁环</span><span>娑ㄨ穼</span><span>娑ㄨ穼%</span></div>
        ...
        {grouped.map(({ group, items }) => (
          <div className="instrument-group" key={group || '_ungrouped'}>
            {group ? <button
              ...
              onDragOver={(event) => { event.preventDefault(); if (draggingItem && dropGroup !== group) setDropGroup(group); }}
              onDragStart={() => setDraggingGroup(group)}
              onDrop={(event) => draggingItem ? void moveItemToGroup(group) : dropGroupOn(event, group)}
              title={`鎷栧姩鍒嗙粍 ${group} 鎺掑簭`}
              type="button"
            ><span>{collapsedGroups.has(group) ? <ChevronRight size={14} /> : <ChevronDown size={14} />}</span><span>{group}</span><span className="group-count">{items.length}</span></button> : null}
      ...
      {directoryOpen ? <InstrumentDirectory groups={groups} instruments={instruments} onChanged={onChanged} onClose={() => setDirectoryOpen(false)} selected={selected} /> : null}
      {managing ? <InstrumentManager groups={groups} instrument={managing} instruments={instruments} onChanged={(nextSelected) => { setManaging(null); onChanged(nextSelected); }} onClose={() => setManaging(null)} /> : null}
    </aside>
  );
}

function groupHeaderClass(group: string, draggingGroup: string, dropGroup: string) {
  return ['group-header', draggingGroup === group ? 'dragging' : '', dropGroup === group ? 'drop-target' : ''].filter(Boolean).join(' ');
}
