import { BellRing, ChevronRight, CloudCheck, Database, ListTree, Megaphone, Package, RadioTower, RefreshCw, Users } from 'lucide-react';
...
import { MobileNotifySettings } from './MobileNotifySettings';
...
export function MobileProfile({ dashboard, liveUpdate, onOpenTranscript }: Props) {
  ...
  const [managingNotify, setManagingNotify] = useState(false);
  ...
  return (
    <div className="mobile-screen-content mobile-profile-screen">
      ...
      <section className="mobile-card mobile-settings-group">
        ...
        <button className="mobile-setting-row" disabled={apkUpdate.installing || (!apkUpdate.hasUpdate && apkUpdate.message !== '')} onClick={() => void apkUpdate.downloadAndInstall()} type="button"><SettingContent icon={apkUpdate.installing ? <RefreshCw className="spinning" size={20} /> : <Package size={20} />} note={apkUpdateNote(apkUpdate)} title="应用安装包更新" /></button>
...
function apkUpdateNote(update: ReturnType<typeof useApkUpdate>): string {
  if (update.installing || update.message) return update.message || '正在检查…';
  if (update.hasUpdate) return '发现新版本，点击下载安装';
  if (update.currentVersion !== '0') return `当前已是最新版 v${update.currentVersion}`;
  return '检查服务器安装包';
}
